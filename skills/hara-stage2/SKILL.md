...
3. `故障描述` 必须直接复制 Stage1 对应故障分析字段原文（即 `Stage1_Fault_Text`），不得重新生成、改写或拼接 `Milf_ID`。
4. 基于功能影响推导车辆级后果，选择 `vehicle_hazards.json` 中最贴近的危害。
5. 为每条 MF 写入 `hazard_reasoning`。
6. 写入 Stage2 单功能片段。
7. 运行 `stage2_slice --fix`；失败时修正当前片段。
8. 所有 Stage2 片段通过后进入 Stage2R；不要在 Stage2R 前合并最终 Stage2 文件。
