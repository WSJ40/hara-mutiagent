...
- 不相关：`不涉及，<理由>`
- 相关：`相关，<理由>`
- 不参与危害事件因果链但影响 SEC：仍写为相关，推荐格式 `相关，不影响危害事件因果链，但影响SEC的S/E/C评级，<理由>`。
- `不涉及` 只能用于字段与其他条件互斥，或该字段对危害事件因果链和 SEC 评级均无影响；例如保留 `特殊要素=维修车间/维修间` 时，不要同时保留 `道路类型=城市支路`，道路类型应改为 `不涉及` 并说明互斥原因。
