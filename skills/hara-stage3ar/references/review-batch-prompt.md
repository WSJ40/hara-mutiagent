...
8. 检查天气、能见度、玻璃起雾等条件是否真的改变当前 MF 到危害事件的链路；若不改变因果链但影响后续 SEC 的 S/E/C 评级，应判为相关，并要求写明“相关，不影响危害事件因果链，但影响SEC的X评级，理由”；只有既不影响因果链也不影响 SEC，或与其他条件互斥时才判为不涉及。
9. 检查互斥场景字段，例如 `道路类型=城市支路` 与 `特殊要素=维修车间/维修间` 不能同时保留；保留维修场所时道路类型应为 `不涉及`，或改写成真实道路场景。
10. 不输出 S/E/C、ASIL、FTTI、安全目标或安全状态；但可以判断某条件是否会影响后续 SEC 评级，从而决定 Stage3A 相关性。
